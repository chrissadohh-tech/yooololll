...
// Exports the same RSProvider interface as providers/deepseek.js and kimi.js;
...
const RSProvider = (() => {
  ...
  // MutationObserver snapshots its joined source into `pre.dataset.rsCode`,
  ...
  function snapshotCode(pre) {
    ...
    if (!live) return;
    const prev = pre.dataset.rsCode || "";
    if (live.length > prev.length) pre.dataset.rsCode = live;
  }
  ...
  function codeText(pre) {
    const cached = pre.dataset.rsCode || "";
    const live = codeLinesText(pre);
  ...
  // `#rs-qwen-net` node as JSON { rid, text, done, t }. netLatest() reads it.
  ...
  function netLatest() {
    try {
      const n = document.getElementById("rs-qwen-net");
  ...
  // Walk element text, skipping .rs-chip and any excluded selector.
  ...
  // Scope to the site's composer only; skip OR's own injected textarea
  // (#rs-set-text inside #rs-root) so login pages without a site editor return
  ...
  // keep #rs-bar in #rs-root (position:fixed) and hug it to the composer's top
  ...
  function setInputLock(on) {
    ...
    if (on) {
      ...
      ed.setAttribute("data-rs-locked", "1");
    } else {
      ed.removeAttribute("readonly");
      ed.removeAttribute("data-rs-locked");
  ...
  function streamText(item) {
    ...
    const bd = bodyEl(item);
    return bd ? textWithout(bd, ".rs-chip") : "";
  }
  ...
  function truncateForSend(text) {
    ...
    const marker =
      `\n\n[…OR: result truncated to fit Qwen's ${SEND_CAP}-character input ` +
  ...
  async function typeAndSend(text, images) {
    ...
    const wasLocked = !!ed.getAttribute("data-rs-locked");
  ...
  // .rs-cmd-mask and let the CSS rule (overlay.css) re-hide recreated pre
  ...
  function findToolBlockSpot(item) {
    ...
    [...bd.children].forEach((el) => {
      if (el.classList.contains("rs-chip") || el.closest(S.codeWrap) || el.querySelector(S.codeWrap)) return;
  ...
  // build onto <html data-rs-qwen-ver>; read it from the page to confirm which
  ...
  function setVersionBeacon() {
    try { document.documentElement.setAttribute("data-rs-qwen-ver", QWEN_VER); } catch {}
  }
  ...
  // tool-calls instead of OR commands. Removed 2026-07: the real cause of
