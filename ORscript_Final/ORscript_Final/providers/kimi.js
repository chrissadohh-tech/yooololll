...
// Exports the same RSProvider interface as providers/deepseek.js and gemini.js;
...
const RSProvider = (() => {
  ...
  // Scope to the SITE's composer only: skip OR's own injected UI (the
  // settings textarea #rs-set-text in #rs-root). On login/OAuth pages with no
  ...
  const getEditor = () => {
    ...
    // barAnchor() then hugged the fixed OR bar over the editor. The real
  ...
  // subtree - inserting our foreign #rs-bar into `.chat-editor` makes Vue's next
  ...
  // the core keeps the bar in its own #rs-root (position:fixed) but positions it
  ...
  function setInputLock(on) {
    ...
    const ed = getEditor();
    if (on) {
      if (ed) { ed.setAttribute("contenteditable", "false"); ed.setAttribute("data-rs-locked", "1"); }
      ...
      if (ed) { ed.setAttribute("contenteditable", "true"); ed.removeAttribute("data-rs-locked"); }
  ...
  // its built-in tools over the OR command protocol. On K3 the only
  ...
  function nativeAgentModeOn() {
    ...
    // its own tooling instead of emitting OR command blocks.
  ...
  function modeWarning() {
    if (nativeAgentModeOn())
      return `Switch the model picker off <b>K3 Swarm</b> (pick <b>K3</b> or <b>Instantané</b>) - ` +
        `Kimi's own agentic mode replaces the OR commands with its native tools and breaks the agent loop.`;
    return "";
  }
  ...
  function overlayBlocking() {
    for (const mask of document.querySelectorAll(MODAL_MASK_SEL)) {
      if (mask.closest("#rs-root")) continue;
  ...
  // Pick a model that actually speaks the OR command protocol, ONCE,
  ...
  // its identity) with .rs-cmd-mask so the overlay.css rule re-hides recreated
  ...
  function findToolBlockSpot(item /*, chip */) {
    ...
    md.querySelectorAll(S.codeWrap).forEach((cw) => {
      if (cw.closest(".rs-chip") || cw.closest(S.thinking)) return; // never a drafted-in-reasoning block
    ...
    [...md.children].forEach((el) => {
      if (el.classList.contains("rs-chip") || el.closest(S.codeWrap) ||
    ...
    md.querySelectorAll(".paragraph, p").forEach((el) => {
      if (el.classList.contains("rs-tool-hide") || el.closest(".rs-chip") ||
  ...
  return {
    ...
    // the capture is shown in the core's left-hand OR popup (ui.showImages).
    supportsVision: true,
    timings,
    ...
    reliableCounts: true,
    // Shown as a permanent, non-intrusive notice in the OR panel.
    // Kimi sometimes reaches for its OWN built-in/native tools (web search, code
    // runner, etc.) instead of emitting the OR command blocks that drive
    ...
    unstableWarning:
      ...
      "If it stops acting in Roblox Studio and answers in plain text or runs its own tools, remind it to use the OR commands - or start a new session.",
