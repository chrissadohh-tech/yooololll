...
const RSProvider = (() => {
  ...
  const S = {
    ...
    // so "robloxscript_….jpg JPG 179 KB Output of 'screen_capture'…" failed to match
  ...
  function textWithout(root, excludeSel) {
    if (!root) return "";
    const skip = S.thinking + ", .rs-chip, " + S.attachment + (excludeSel ? ", " + excludeSel : "");
    let t = "";
    const walk = (n) => {
  ...
  // reconciles the composer subtree, so we must NOT insert our #rs-bar into it
  ...
  // bar in #rs-root, positions it to hug that card's top edge at full width, and
  ...
  // identity) with .rs-cmd-mask so a CSS rule re-hides recreated wrappers with
  ...
  function findToolBlockSpot(item /*, chip */) {
    ...
    for (const cc of item.querySelectorAll(S.copyCodeBtn)) {
      ...
      if (!wrap || wrap.closest(".rs-chip")) continue;
      if (CMD_SHAPE.test(wrap.textContent || "")) {
        wrap.classList.add("rs-tool-hide");
    ...
    [...reply.children].forEach((el) => {
      if (el.classList.contains("rs-chip") || el.closest(S.thinking) ||
