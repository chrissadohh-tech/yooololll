...
const RSProvider = (() => {
  ...
  // Bar sits *above* the rounded composer surface, not inside where you type.
  // The surface (element carrying --composer-surface-primary) previously hosted
  // the bar as its first child in the `header` grid row — but that put the bar
  // inside the same rounded card as the textarea, overlapping its padding and
  // bleeding into the typing area (like DeepSeek/Crax before their fix).
  // Instead anchor it right before the card in its parent, so it is a clean
  // header above the box — consistent with copilot/crax (parent: card.parentElement,
  // before: card, inside:false).
  // Anchored mode: React re-renders the composer grid on every keystroke,
  // fighting any node we insert into it. Keep the bar in #rs-root and hug
  // the composer card's top edge from outside instead.
  function barAnchor() {
    const ed = getEditor();
    if (!ed) return null;
  ...
  function streamText(item) {
    return item ? textWithout(item, ".rs-chip") : "";
  }
  ...
  function readAssistant() {
    ...
    return {
      present: true,
      reply: mds.map((m) => textWithout(m, ".rs-chip")).join("\n").trim(),
  ...
  function truncateForSend(text) {
    ...
    const marker = (what) =>
      `\n\n[…OR: result truncated (${what}) so it can be pasted into ` +
  ...
  const PROMPT_EXTRA = `- WHEN THE USER SENDS AN IMAGE: by default it is REFERENCE MATERIAL for the work they want done in their project - a screenshot of a bug, a mockup of the UI they want, a photo of the thing to build, a picture of what is wrong in Studio. Look at it, use it to understand what they want, and then do that work with the OR commands. Do NOT generate a new image from it, and do NOT treat it as an image-editing request. Only generate an image when the user EXPLICITLY asks you to create, generate, draw or edit one ("make me an image of...", "generate a texture", "edit this picture"). If what they want from the image is genuinely unclear, ask them in one short sentence rather than guessing - and never guess "they want a picture".`;
  ...
  return {
    ...
    init({ diag: d } = {}) {
      ...
      try { document.documentElement.setAttribute("data-rs-gpt-ver", "2026-08-13_cmtap+toolkey"); } catch {}
    },
    // turns
