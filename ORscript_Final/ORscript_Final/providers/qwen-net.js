...
// that carry OR commands) in a Monaco editor that DISPOSES off-screen
...
// done flag into a DOM node (#rs-qwen-net, JSON textContent) that the isolated
...
(() => {
  "use strict";
  if (window.__rsQwenNet) return;
  window.__rsQwenNet = true;

  const NODE_ID = "rs-qwen-net";
  const node = () => {
