...
const RSProvider = (() => {
  ...
  // OR bar stuck on "Agent active" with no way to start a new session.
  ...
  // Gemini's Angular reconciles the composer subtree, so inserting #rs-bar
  // into it breaks the next diff. Use barAnchor() instead — the core's
  // anchored branch keeps the bar in our own #rs-root and hugs the
  // input-area-v2 top edge from outside. Never returns a mount.
  // Fallbacks guard against DOM renames (tag → div, class shifts) so the
  // bar never falls back to floating and sticks out.
  function barAnchor() {
    return document.querySelector(S.inputArea)
        || document.querySelector("rich-textarea")
        || document.querySelector(".ql-editor")?.closest("input-area-v2, rich-textarea, div")
        || null;
  }
  ...
  function setInputLock(on) {
    ...
    if (on) ph.setAttribute("data-rs-locked", "1");
    else ph.removeAttribute("data-rs-locked");
  }
  ...
  // function's real job is applying the .rs-tool-hide classes correctly.
  ...
  function findToolBlockSpot(item /*, chip */) {
    ...
    for (const mc of replies) {
      ...
      mc.querySelectorAll(S.codeWrap).forEach((cb) => {
        if (cb.closest(".rs-chip")) return;
        if (CMD_SHAPE.test(cb.textContent || "")) {
          cb.classList.add("rs-tool-hide");
          ...
          // rule `message-content.rs-cmd-mask code-block` keeps every
          // recreated block hidden with zero flash.
          mc.classList.add("rs-cmd-mask");
