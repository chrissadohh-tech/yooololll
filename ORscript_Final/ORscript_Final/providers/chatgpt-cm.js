...
//     .rs-tool-hide / .rs-cmd-mask) and re-measuring after a layout + 800ms gave
...
//
// Protocol: the isolated world dispatches a "rs-cm-sync" CustomEvent on
...
(() => {
  "use strict";
  if (window.__rsChatgptCm) return;
  window.__rsChatgptCm = true;

  const ATTR = "data-rs-cm";
  const LEN = "data-rs-cm-len";
  ...
  document.addEventListener("rs-cm-sync", () => { try { syncAll(); } catch {} }, true);
