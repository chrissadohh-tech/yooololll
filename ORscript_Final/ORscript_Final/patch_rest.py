...
# ── gemini: logs the result but sends anyway ─────────────────────────────────
...
# ── meta + arena: they already read `ok`, they just never act on it ──────────
for p, tag, helper in (("providers/meta.js", "meta", "hasPendingAttachment"),
                       ("providers/arena.js", "arena", "hasPendingAttachment")):
    s = load(p)
    if tag == "meta":
        old = '''      try {
      ...
      if (!ok) {
        ...
        // a message that claims a picture the model cannot see is worse than an error.
        try { ok = await attachImages(images); if (ok) _attachedImages = images; } catch { ok = false; }
      }
      if (!ok && !%s()) { %s }''' % (helper, MSG)
    else:
        old = '''      try {
        const ok = await attachImages(images);
        if (ok) _attachedImages = images;
      ...
      if (!ok && !%s()) { %s }''' % (helper, MSG)
    s = swap(s, old, new, p); save(p, s); print(tag, "patched")

# ── ollama: silent catch, no spaces ──────────────────────────────────────────
