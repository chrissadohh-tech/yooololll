...
const RS = (() => {
  ...
  function toolCategory(name) {
    ...
    if (/^(multi_edit|insert_from_creator_store|store_image)$/.test(n) || n === "execute_luau")
      return "edit";
    // The three screenshot commands are written PascalCase; compare normalized so a
    // model echoing viewport_screenshot_roblox still gets the screen chip.
    const key = String(n || "").toLowerCase().replace(/[^a-z0-9]/g, "");
    if (n === "screen_capture" || key === "viewportscreenshotroblox" || key === "viewportscreenshotblender" ||
      key === "viewportscreenshot" || n === "shot_test" ||
      n === "attach_feedback" || n === "attachfeedbackor" || n === "attach_image" || n === "attach_images" ||
      n === "attach_screenshot" || n === "attach_file" || n === "attach_last_screenshot" ||
      n === "copy_screenshot" || n === "paste_screenshot" ||
      n === "attach_check" || n === "attachment_check" || n === "or_report" || n === "bug_report" || n === "or_diagnostics") return "screen";
...
- Your FIRST action: list_commands {} - then use ONLY exact names/params from that reference; never guess. If a command is not listed there, it DOES NOT EXIST — do not invent names (get_studio_list and friends are not real, and the one real listing tool, list_roblox_studios, belongs to the screenshot path - see the STUDIO ID line below).
- STUDIO ID (the screenshot path): the Roblox MCP's screen_capture declares capture_id AND studio_id required, and it REFUSES an invented id — its own words are "The requested studio_id is not connected ... Call list_roblox_studios for the current ...". OR fills both for you: call ViewportScreenshotRoblox {} and the CONNECTED id is looked up with list_roblox_studios and sent automatically. That lookup is a plain MCP call - no PowerShell, no script file written to disk - so an antivirus that blocks scripts has nothing to stop. NEVER invent a studio_id and never ask the user for one. For a dummy, call npc_spawn_pathfinding directly (e.g. {"command":"npc_spawn_pathfinding","params":{"rig_name":"Dummy"}}).
...
- STUDIO ID: never hallucinate one and never ask the user for it. When a Studio tool of yours genuinely needs it, get the CONNECTED one from \`list_roblox_studios\` - an invented id is refused with "The requested studio_id is not connected ... Call list_roblox_studios for the current ...". The screenshot commands do exactly that themselves, which needs no PowerShell and no script file, so security software has nothing to block. The ONLY discovery tools you may call are \`list_commands\` (Roblox), \`list_mcp_servers\` (other MCP servers) and \`list_roblox_studios\` (studio ids only). For a dummy/NPC from the toolbox, call \`npc_spawn_pathfinding\` directly — e.g. \`{"command":"npc_spawn_pathfinding","params":{"rig_name":"Dummy","behavior":"follow"}}\` or with \`target\` — and do NOT ask the user to run anything.
...
- WEB: \`web_search\` {query, limit?} searches the web from the extension itself (it walks several search backends and reports which one answered) and returns titles, URLs and snippets; \`web_fetch\` {url?, query?, max_chars?} downloads a page and strips it to readable text - pass query instead of url to search first and fetch the best hit. Both run in the background service worker, so they are NOT bound by the page's CORS rules: use them for docs/reference BEFORE asking the user to paste anything. If one fails it lists every backend it tried and why - quote that instead of guessing.\n- SCREENSHOT: THREE separate commands, no aliases, and each one photographs ONE thing. \`ViewportScreenshotRoblox\` {} takes a picture INSIDE Roblox Studio and needs only Studio to be RUNNING - no window, no focus, and it does not matter which window is in front; \`ViewportScreenshotBlender\` {} is inside Blender (Blender must be running and connected); \`ViewportScreenshot\` {} is the OVERALL one - Studio if it is running, else Blender, else the whole screen (every monitor, no app needed). None of the three ever substitutes another app: if its app is not there, the command says so and takes no picture. The picture is attached to your next message so you can see it AND copied to the system clipboard so the user can paste it by hand. There is no or_screenshot / screenshot / take_screenshot / send_screenshot / screen_capture / window / tab / auto command - they do not exist, never call them and never invent one. If a capture reports NOTHING it names the reason (a Studio miss usually means Studio is not running, or its MCP is offline): quote that line instead of guessing, and after a second failure use a text command (inspect_instance, get_studio_state, script_analysis) rather than retrying the same command.\n- ATTACHING IMAGES: \`attach_feedback\` {} re-sends the MOST RECENT capture as an attachment on THIS message AND copies it to the system clipboard so the user can paste it (Ctrl+V) anywhere. \`{index:N}\` picks an older capture (0 = newest), \`{path:\"C:/.../ref.png\"}\` attaches a file from disk instead, \`{copy:true, send:false}\` copies only and attaches nothing, \`{paste:true}\` stages it in the composer without sending, \`{source:\"studio\"|\"blender\"|\"desktop\"}\` takes a fresh capture first. Use it to look at a screenshot twice, to pull in a reference image the user mentions, or whenever the user asks to copy/paste an image. Aliases: attachfeedbackor, attach_image, attach_images, attach_file, attach_screenshot, attach_last_screenshot, attach_recent_image, copy_screenshot, paste_screenshot.\n- STUDIO PLUGINS: \`plugin_list\` lists PluginGuiService / CoreGui plugin widgets / ServerStorage.RobloxScript.Plugins; \`plugin_create\` {name} writes a local-plugin skeleton (toolbar + DockWidget) you can Save as Local Plugin; \`plugin_inspect\` {path} reads one plugin instance.\n- SCRIPT ANALYSIS: \`script_analysis\` {scope?, include_output?, max_scripts?} reads Studio scripts (LuaSourceContainer), syntax-checks with loadstring, flags common lints, and returns the Output panel (LogService:GetLogHistory). Use this instead of asking the user to copy Script Analysis / Output.
...
IMPORTANT: Your very first action is to write \`list_commands\` to get the full command reference - never guess a command name or parameter. After receiving the result, reply with exactly one short sentence confirming you are ready, then wait for the user's first request.${providerNotes ? "\n\n" + providerNotes : ""}`;
  ...
  const TOOL_NOTES = {
    ...
    attach_check:
      "Checks whether THIS chat site accepts attachments, in plain words. It stages a 1x1 test " +
      "picture through the page's OWN upload path, reports whether the site's file picker takes " +
      "pictures and/or documents, then removes the test picture again - nothing is typed, nothing " +
      "is sent, so it is safe to run any time. Run it when the user asks whether images or files " +
      "can be attached here, when an attachment failed, or before promising a screenshot will " +
      "arrive. Aliases: attachment_check, attach_compat, attach_support, attach_test, " +
      "provider_attach_check. Machine-readable line: OR_ATTACH_CHECK {...}",
    or_report:
      "The ONE command to run when anything is wrong - run it BEFORE asking the user to describe " +
      "the problem. One paste-ready block: OR version, engine, provider, vision, page, bridge " +
      "state, the agent build (and whether a screenshot can reach you), attachment capability, the " +
      "last capture, every uncaught page error captured since the page loaded (with the function " +
      "and line), and the tail of the diagnostic trail. Quote it whole. Aliases: bug_report, " +
      "or_bug_report, support_bundle, or_diagnostics. Machine-readable line: OR_REPORT {...}",
    ViewportScreenshotRoblox:
      ...
      "Never call or_screenshot / screenshot / take_screenshot / send_screenshot / screen_capture - they are not OR commands. " +
      "The picture also lands on the system clipboard, so the user can paste it with Ctrl+V. " +
      "NEVER accept a picture that is not the thing asked for: if the result says the capture came from somewhere else, or that " +
      "no image came back, say so and switch to a text command (inspect_instance / get_studio_state / script_analysis) " +
      "instead of retrying the same capture.",
    ViewportScreenshotBlender:
      "Takes a picture INSIDE Blender and ATTACHES the image to your next message so you can actually see it. " +
      "Needs Blender to be RUNNING and connected (Menu → Connect Blender). It NEVER photographs anything else: " +
      "if Blender is not connected it says so, and you report that instead of pretending you have a picture. " +
      "Never call or_screenshot / screenshot / take_screenshot / send_screenshot / screen_capture - they are not OR commands. " +
      "The picture also lands on the system clipboard, so the user can paste it with Ctrl+V. " +
      "NEVER accept a picture that is not the thing asked for: if the result says the capture came from somewhere else, or that " +
      "no image came back, say so and switch to a text command (inspect_instance / get_studio_state / script_analysis) " +
      "instead of retrying the same capture.",
    ViewportScreenshot:
      "The OVERALL screenshot: ATTACHES the best available picture to your next message - Roblox Studio if it is running, " +
      "else Blender if it is connected, else the whole screen (every monitor, no app needed). Use it when the user simply " +
       "The whole-screen fallback is the only route that needs PowerShell (studio_shot.ps1); if security software blocks that script, OR says so in one line and never retries it - " +
       "the Studio and Blender commands still work because neither needs a script. " +
      "wants to see their screen; use ViewportScreenshotRoblox or ViewportScreenshotBlender when the app matters. " +
      "The result names which app was photographed - quote that line so the user knows what they are looking at. " +
      "Never call or_screenshot / screenshot / take_screenshot / send_screenshot / screen_capture - they are not OR commands. " +
      "The picture also lands on the system clipboard, so the user can paste it with Ctrl+V. " +
      "NEVER accept a picture that is not the thing asked for: if the result says no image came back, " +
      "say so and switch to a text command (inspect_instance / get_studio_state / script_analysis) instead of retrying.",
    attach_feedback:
...
If anything in the image is ambiguous, make the most reasonable interpretation and note it in one clause - do not stall asking.${notes ? "\nUser notes: " + notes : ""}`;
  ...
  // ── Which chat providers can actually attach, and do they see pictures? ──────
  // The user's question ("does this provider support file attachment?") has a live
  // answer from attach_check, but the useful comparison is across providers - so the
  // facts sit here as data rather than as prose that goes stale. test-v118 re-reads
  // every providers/*.js on each run and fails if a row stops matching the file it
  // describes, which is what makes this table trustworthy rather than a snapshot.
  //
  // images: the provider implements attachImages (the ONE upload path OR has).
  // vision: supportsVision is not explicitly false. A picture can be ATTACHED even
  //         when the model cannot read it - those two facts are reported apart.
  const PROVIDER_ATTACH_MATRIX = [
    { id: "arena",    name: "Arena",       images: true, vision: true },
    { id: "chatgpt",  name: "ChatGPT",     images: true, vision: false },
    { id: "claude",   name: "Claude",      images: true, vision: true },
    { id: "copilot",  name: "Copilot",     images: true, vision: true },
    { id: "crax",     name: "Crax GPT",    images: true, vision: true },
    { id: "deepseek", name: "DeepSeek",    images: true, vision: true },
    { id: "freebuff", name: "Freebuff",    images: true, vision: true },
    { id: "gemini",   name: "Gemini",      images: true, vision: true },
    { id: "glm",      name: "GLM",         images: true, vision: true },
    { id: "kimi",     name: "Kimi",        images: true, vision: true },
    { id: "meta",     name: "Meta AI",     images: true, vision: true },
    { id: "ollama",   name: "Ollama",      images: true, vision: false },
    { id: "oxalpha",  name: "Ox Alpha",    images: true, vision: true },
    { id: "qwen",     name: "Qwen",        images: true, vision: true },
    { id: "useai",    name: "Use AI",      images: true, vision: true },
  ];
