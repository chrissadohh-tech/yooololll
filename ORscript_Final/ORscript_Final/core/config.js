...
const RS = (() => {
  ...
  function toolCategory(name) {
    ...
    if (/^(multi_edit|insert_from_creator_store|store_image)$/.test(n) || n === "execute_luau")
      return "edit";
    if (n === "screen_capture" || n === "or_screenshot" || n === "screenshot" || n === "take_screenshot" || n === "send_screenshot" || n === "attach_feedback" || n === "attachfeedbackor" || n === "attach_image" || n === "attach_images" || n === "attach_check" || n === "attachment_check" || n === "or_report" || n === "bug_report" || n === "or_diagnostics" || n === "attach_screenshot" || n === "attach_file" || n === "attach_last_screenshot" || n === "copy_screenshot" || n === "paste_screenshot" ||
...
- WEB: \`web_search\` {query, limit?} searches the web from the extension itself (it walks several search backends and reports which one answered) and returns titles, URLs and snippets; \`web_fetch\` {url?, query?, max_chars?} downloads a page and strips it to readable text - pass query instead of url to search first and fetch the best hit. Both run in the background service worker, so they are NOT bound by the page's CORS rules: use them for docs/reference BEFORE asking the user to paste anything. If one fails it lists every backend it tried and why - quote that instead of guessing.\n- SCREENSHOT: \`or_screenshot\` {target?:"auto"|"studio"|"blender"|"desktop"|"window"|"tab", focus?, max_width?} captures a screenshot and attaches the image to your next message so you can see it. The three the user asks for: "studio" = Studio takes its own picture (needs only Studio open - no window, no focus), "blender" = the Blender viewport via the Blender addon, "desktop" (aliases screen, pc, os) = the WHOLE PC, every monitor, captured from the desktop itself and needing no window in front. "auto" tries the MCP capture, then the Blender viewport, then the Roblox Studio WINDOW (photographed through the agent - works while the browser is in front and needs no page permission), then this tab. \`or_focus_studio\` {} brings the Studio window to the front on Windows (it steals focus - opt-in only). Aliases: screenshot, take_screenshot, send_screenshot. If a capture reports NOTHING, quote the per-target reason it returns (a Studio miss usually means the MCP is offline - check list_mcp_servers - not that Studio is empty; a tab miss usually means the tab in FRONT is not this chat or is a chrome:// page), then switch to target:"window" (or target:"desktop" for the whole screen) or to a text command instead of retrying the same target. Every reason it gives says WHERE each picture came from, so quote that line instead of guessing.\n- ATTACHING IMAGES: \`attach_feedback\` {} re-sends the MOST RECENT capture as an attachment on THIS message AND copies it to the system clipboard so the user can paste it (Ctrl+V) anywhere. \`{index:N}\` picks an older capture (0 = newest), \`{path:\"C:/.../ref.png\"}\` attaches a file from disk instead, \`{copy:true, send:false}\` copies only and attaches nothing, \`{paste:true}\` stages it in the composer without sending, \`{source:\"studio\"|\"tab\"|\"blender\"}\` takes a fresh capture first. Use it to look at a screenshot twice, to pull in a reference image the user mentions, or whenever the user asks to copy/paste an image. Aliases: attachfeedbackor, attach_image, attach_images, attach_file, attach_screenshot, attach_last_screenshot, attach_recent_image, copy_screenshot, paste_screenshot.\n- STUDIO PLUGINS: \`plugin_list\` lists PluginGuiService / CoreGui plugin widgets / ServerStorage.RobloxScript.Plugins; \`plugin_create\` {name} writes a local-plugin skeleton (toolbar + DockWidget) you can Save as Local Plugin; \`plugin_inspect\` {path} reads one plugin instance.\n- SCRIPT ANALYSIS: \`script_analysis\` {scope?, include_output?, max_scripts?} reads Studio scripts (LuaSourceContainer), syntax-checks with loadstring, flags common lints, and returns the Output panel (LogService:GetLogHistory). Use this instead of asking the user to copy Script Analysis / Output.
