...
const RS = (() => {
  ...
  function toolCategory(name) {
    ...
    if (/^(multi_edit|insert_from_creator_store|store_image)$/.test(n) || n === "execute_luau")
      return "edit";
    if (n === "screen_capture" || n === "or_screenshot" || n === "shot_test" ||
...
IMPORTANT: Your very first action is to write \`list_commands\` to get the full command reference - never guess a command name or parameter. After receiving the result, reply with exactly one short sentence confirming you are ready, then wait for the user's first request.${providerNotes ? "\n\n" + providerNotes : ""}`;
  ...
  const TOOL_NOTES = {
    ...
    or_screenshot:
      "Takes a screenshot and ATTACHES the image to your next message so you can actually see it. " +
      "ONE command, exactly THREE targets, no aliases: {} or {\"target\":\"studio\"} is a picture taken INSIDE Roblox Studio, " +
      "which needs only Studio to be open - no window, no focus, and it does not matter which window is in front; " +
      "{\"target\":\"blender\"} is inside Blender; {\"target\":\"desktop\"} is the whole PC, every monitor. " +
      "Never call screenshot / take_screenshot / send_screenshot / screen_capture / window / tab - they are not OR commands. " +
